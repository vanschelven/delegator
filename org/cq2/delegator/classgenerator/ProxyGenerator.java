...
public class ProxyGenerator extends ClassLoader implements Constants {
	...
	private final MethodFilter defaultMethodFilter = new MethodFilter() {
	...
	private void addMethodsFromSuperClass() {
		...
		Method[] methods =
			(Method[]) Util.getMethods(superClass, defaultMethodFilter).toArray(
				new Method[0]);
		//printArray(methods);
	...
	private MethodGen addMethodHeader(Method method, Type returnType, Class firstArg) {
		...
		MethodGen methodGen =
			new MethodGen(
				method.getModifiers() & ~(Modifier.NATIVE | Modifier.ABSTRACT),
				returnType,
				(Type[]) types.toArray(new Type[] {}),
				generateParameterNames(types.size()),
				method.getName(),
				classGen.getClassName(),
				instrList,
				constPool);
	...
	private void createParameterArray(Class[] argTypes) {
		instrList.append(new PUSH(constPool, argTypes.length)); // array size
		instrList.append(instrFact.createNewArray(Type.OBJECT, (short) 1));
		int stackIndex = 1;
		for (int i = 0; i < argTypes.length; i++) {
			Class c = argTypes[i];
			instrList.append(InstructionConstants.DUP); // array ptr
			instrList.append(new PUSH(constPool, i)); // array index
			if (c.isPrimitive()) {
			...
			instrList.append(InstructionConstants.AASTORE); // set array element
		}
	}
	...
	private int createPrimitiveTypeClass(
		String className,
	...
	private void createParameterTypeArray(Class[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			instrList.append(InstructionConstants.DUP); // array pointer
	...
	public static Object newProxyInstance(
		...
		try {
			Class proxyClass =
				classLoader.loadClass(ProxyGenerator.getProxyClassName(theInterface));
			return ProxyGenerator.getInstance(proxyClass, handler);
		} catch (ClassNotFoundException e) {
			return new ProxyGenerator(classLoader, theInterface).getInstance(handler);
		}
	}
