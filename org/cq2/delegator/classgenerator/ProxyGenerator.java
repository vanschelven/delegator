...
public class ProxyGenerator implements Constants {
  ...
  static class ClassInjector extends ClassLoader {
    ...
    protected Class findClass(String classname) throws ClassNotFoundException {
      if (classname.endsWith("$component")) {
        ...
        return injectProxyClass(loadClass(classname.substring(0, classname
            .length() - 6)));
    ...
    private Class injectComponentClass(Class clazz) {
      ...
      byte[] classDef = new ProxyGenerator(className, clazz, Component.class)
          .generateComponent();
    ...
    private Class injectClass(Class clazz, String prefix, Class marker) {
      ...
      byte[] classDef = new ProxyGenerator(className, clazz, marker)
          .generateProxy();
      return inject(className, classDef, clazz.getProtectionDomain());
    }
  }
  ...
  private static ClassInjector injector = ClassInjector.create(ClassLoader
  ...
  private void addDelegationMethods(MethodFilter methodFilter, boolean useSelf) {
  ...
  private void addSelfField() {
    FieldGen fieldGen = new FieldGen(ACC_PUBLIC | ACC_TRANSIENT,
        new ObjectType("java.lang.reflect.InvocationHandler"), "self", classGen
            .getConstantPool());
    classGen.addField(fieldGen.getField());
  }

  private void addDefaultConstructor() {
    MethodGen methodGen = new MethodGen(ACC_PUBLIC, Type.VOID, Type.NO_ARGS,
  ...
  private MethodGen addMethodHeader(Method method, Type returnType,
    ...
    MethodGen methodGen = new MethodGen(newMods, returnType, (Type[]) types
        .toArray(new Type[] {}), generateParameterNames(types.size()), method
        .getName(), classGen.getClassName(), instrList, constPool);
    return methodGen;
  }
  ...
  private void createCallToSuper(Method method, Type returnType, int stackIndex) {
    ...
    instrList.append(instrFact.createInvoke(method.getDeclaringClass()
        .getName(), method.getName(), returnType, getArgumentTypes(method),
  ...
  private void createCallToInvocationHandler(Method method, boolean useSelf) {
    ...
    if (useSelf) {
      ...
      instrList.append(instrFact.createFieldAccess(classGen.getClassName(),
          "self", new ObjectType("java.lang.reflect.InvocationHandler"),
          Constants.GETFIELD));
    } else {
      instrList
          .append(instrFact.createFieldAccess("org.cq2.delegator.Self", "self",
              new ObjectType("java.lang.ThreadLocal"), Constants.GETSTATIC));
      instrList.append(instrFact.createInvoke("java.lang.ThreadLocal", "get",
    ...
    instrList.append(instrFact.createInvoke("java.lang.Class",
        "getDeclaredMethod", new ObjectType("java.lang.reflect.Method"),
        new Type[] { Type.STRING, new ArrayType(CLASS, 1) },
    ...
    instrList.append(instrFact.createInvoke(
        ...
        new Type[] { Type.OBJECT, new ObjectType("java.lang.reflect.Method"),
  ...
  private void createParameterArray(Class[] argTypes) {
    ...
    for (int i = 0; i < argTypes.length; i++) {
      ...
      if (c.isPrimitive()) {
        ...
        instrList
            .append(InstructionFactory.createLoad(Type.OBJECT, stackIndex)); // arg
                                                                                                                                // i
        stackIndex += 1;
      }
  ...
  private void createClassReference(Class clazz) {
    if (clazz.equals(Integer.TYPE)) {
      ...
      instrList.append(instrFact.createInvoke("java.lang.Class", "forName",
  ...
  public static Proxy newProxyInstance(Class clazz, InvocationHandler handler) {
    try {
      Object proxy = injector.loadClass(getClassName(clazz, "proxy"))
          .newInstance();
  ...
  public static Component newComponentInstance(Class clazz) {
    try {
      return (Component) injector.loadClass(getClassName(clazz, "component"))
          .newInstance();
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
